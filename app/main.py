...
def get_appointment_status(date: datetime.date, start_time: datetime.time, end_time: datetime.time, db: Session = Depends(get_db)):
    """
...
def get_appointments_day(date: datetime.date, db: Session = Depends(get_db)):
    """
...
def cancel_appointment(user_id: str, date : datetime.date, db = Depends(get_db)):
    """
...
def check_appointment_valid(appointment: schemas.AppointmentCreate):
    ...
    if datetime.datetime.combine(appointment.date, appointment.start_time) < datetime.datetime.now():
